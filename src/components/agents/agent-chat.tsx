...
import { Send, Bot, User, Play, StopCircle, Loader, ListTodo } from 'lucide-react';
...
export function AgentChat({ agent, onStatusChange }: AgentChatProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await AgentExecution.sendMessage(agent.id, userMsg.content);
    ...
    <Card className="flex flex-col h-[600px]">
      ...
      <CardFooter className="pt-0">
        <div className="flex items-center w-full gap-2">
          <Input
            placeholder={isActive ? "Type a message..." : "Start the agent to chat"}
            value={inputMessage}
