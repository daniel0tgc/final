...
function MemoryCard({ memory, onBookmarkToggle, bookmarked }: MemoryCardProps) {
  ...
  return (
    <div
      className={`border rounded-lg p-3 mb-2 flex flex-col gap-2 ${getMemoryBadgeColor(
        memory.type
      )}`}
    >
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => onBookmarkToggle(memory.id)}
      ...
      <div className="text-xs text-muted-foreground text-right">
        {format(new Date(memory.timestamp), "MMM d, h:mm a")}
      </div>
    </div>
  );
}
...
export function AgentMemoryView({ agent }: AgentMemoryViewProps) {
  ...
  const refreshMemories = async () => {
    setLoading(true);
    try {
      const allMemories = await AgentMemory.getMemories(agent.id);
  ...
  }, [searchTerm, activeFilter, showBookmarkedOnly, bookmarked, memories]);
  ...
  const handleClearMemories = async () => {
    if (
      confirm(
        "Are you sure you want to clear all memories for this agent? This cannot be undone."
      )
    ) {
      await AgentMemory.clearMemories(agent.id);
      refreshMemories();
    }
  };
      ...
      <div className="px-4 pb-2">
        ...
        >
          <TabsList className="grid grid-cols-6">
            ...
            <TabsTrigger value="message_received">Received</TabsTrigger>
          </TabsList>
              ...
              {filteredMemories.length === 0 ? (
                ...
                filteredMemories.map((memory) => (
                  <MemoryCard
                    ...
                    bookmarked={bookmarked.has(memory.id)}
                  />
                ))
              )}
            </ScrollArea>
          </TabsContent>
          <TabsContent value="user_message">
            <ScrollArea className="h-[400px]">
              {filteredMemories.filter((m) => m.type === "user_message")
                .length === 0 ? (
                <div className="text-center text-muted-foreground py-8">
          ...
          <TabsContent value="agent_response">
            <ScrollArea className="h-[400px]">
              {filteredMemories.filter((m) => m.type === "agent_response")
                .length === 0 ? (
                <div className="text-center text-muted-foreground py-8">
