...
export function AgentCollaboration() {
  ...
  const loadAgentStates = async () => {
    try {
      const states = await fetch("/api/agent-states").then((res) => res.json());
  ...
  const loadCollaborativeTasks = async () => {
    try {
      ...
      for (const agent of agents) {
        const agentTasks = await fetch(`/api/agent/${agent.id}/tasks`).then(
          (res) => res.json()
        );
      ...
      const uniqueTasks = allTasks.filter(
        (task, index) => allTasks.findIndex((t) => t.id === task.id) === index
      );
  ...
  const sendMessage = async () => {
    try {
      const agentObj = agents.find(
        (a: Agent) => a.id === messageForm.sourceAgent
      );
      if (!agentObj) throw new Error("Source agent not found");
      await agentToolRegistry.SEND_MESSAGE(
        {
          to_id: messageForm.targetAgent,
  ...
  const createTask = async () => {
    try {
      const agentObj = agents.find((a: Agent) => a.id === taskForm.assignedTo);
      if (!agentObj) throw new Error("Assigned agent not found");
      await fetch(`/api/agent/${agentObj.id}/create-task`, {
        method: "POST",
                  ...
                  {collaborativeTasks.map((task) => (
                    <div key={task.id} className="p-3 border rounded-lg">
                      ...
                      <div className="text-xs text-gray-500">
                        Assigned to: Agent-{task.assigneeAgentId} • Created:{" "}
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium">
                        {approval.itemType === "task" ? "Task" : "Message"}{" "}
                        Approval Required
                      </h4>
                  ...
                  <div key={agentId} className="p-3 border rounded-lg">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Badge
                          className={
                            state.workload === "idle"
                              ? "bg-gray-500"
                              : state.workload === "light"
                              ? "bg-blue-500"
                              : state.workload === "medium"
                              ? "bg-yellow-500"
                              : state.workload === "heavy"
                              ? "bg-orange-500"
                              : "bg-red-500"
                          }
                        >
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      <p>
                        ...
                        {state.capabilities.join(", ") || "None specified"}
                      </p>
                      <p>
                        <strong>Success Rate:</strong>{" "}
                        {(state.performance.successRate * 100).toFixed(1)}%
                      </p>
                      <p>
